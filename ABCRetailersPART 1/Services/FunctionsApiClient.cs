...
public class FunctionsApiClient : IFunctionsApi
{
    ...
    private static Order ToOrder(OrderDto d)
    {
        return new Order
        {
            // Table keys
            PartitionKey = "Order",
            RowKey = d.Id,                 // maps DTO Id -> RowKey
                                           // Domain fields
            CustomerId = d.CustomerId,
            ProductId = d.ProductId,
            ProductName = d.ProductName,
            Quantity = d.Quantity,
            UnitPrice = (double)d.UnitPrice,                // decimal -> double
            TotalPrice = (double)d.UnitPrice * d.Quantity,  // or use a DTO Total if you add one
            OrderDate = d.OrderDateUtc.UtcDateTime,         // DateTimeOffset -> DateTime (UTC)
            Status = d.Status                                // keep string status
        };
    }
